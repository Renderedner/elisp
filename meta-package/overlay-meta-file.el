...
(defun brnm-activate-meta-file (num)
  ...
  "
  (if (not num)
      (if (and buffer-file-name (string= (f-expand (buffer-file-name)) (f-expand brnm-index-to-meta-files-file)))
	  (save-excursion (mapcar #'brnm-put-overlay-before-sexp-and-number-position
...
(defun brnm-get-regexp-positions-numbered-2 (regexp)
  ()
  "Returns the positions of the matches of regexp in the current buffer.
   The position returned are structured as in:

   <N'th block><N'th subblock><N'th sexp>
   30200 will be the 3rth block 3rd subblock 1st sexp.   
  "
  (let ((break-parag-pos t) (regexp-pos t) (break-subparag-pos t)
	(group-num 1)   (num 0)   (subgroup-num 0)
	(anchor (point-min))
	(return-value nil)
	(count 1))
	...
	(setq break-parag-pos nil))
	  ...
	  (setq break-subparag-pos `(,(match-beginning 0) . ,(match-end 0)))
	(setq break-subparag-pos nil))
	...
	(setq regexp-pos nil))
	  ...
	  (if (< (car break-parag-pos) (car regexp-pos))
	    ...
	    (when (< (car break-subparag-pos) (car regexp-pos))
	      (setq subgroup-num (+ 1 subgroup-num))
	      (setq num                          0)
	      )
	    (setq return-value (cons `(,(substring-no-properties (match-string 0)) .
				       ,(+ (* 10000 group-num) (* 100 subgroup-num) num))
				     return-value))
	    (setq num (+ 1 num))
	    (setq anchor (cdr regexp-pos))
	    )))
...
;; (eek "C-u 1 M-x brnm-activate-meta-file RET")
;; (find-wset "13o___o" '(find-file _mc-current-meta-file) '(brnm-get-regexp-positions-numbered-2 "^(.+)") '(find-ebuffer "*Messages*"))
...
(defun brnm-put-overlay-before-sexp-and-number-position (sexp-number)
  ()
  (save-excursion
    (save-window-excursion
      (let ((overlay-list nil) ovl color)
	(goto-char (point-min))
	(search-forward-regexp (concat "^" (regexp-quote (car sexp-number))) nil t)
	(setq        ovl (make-overlay (match-beginning 0) (match-end 0)))

	(setq color (nth (- (mod (/ (cdr sexp-number) 100) (list-length brnm-color-sequence)) 1) brnm-color-sequence))
	(message color)

	(overlay-put ovl 'before-string (propertize (concat "[" (number-to-string (cdr sexp-number)) "]") 'face `(:height 0.8 :foreground ,color))) ;; :background color
	;; (overlay-put ovl 'face `(:underline ,color))
	ovl
	)))
  )
...
(defun brnm-helpfull-overlays (arg)
  (interactive "P")
  (let ((position-func (if arg
			   #'brnm-get-regexp-positions-numbered-2
			 #'brnm-get-regexp-positions-numbered)))
    (save-window-excursion
      (save-excursion
	(if (string= (f-expand (buffer-file-name)) (f-expand _mc-current-meta-file))
	    (mapcar #'brnm-put-overlay-before-sexp-and-number-position
		    (funcall position-func "^(.+)"))
