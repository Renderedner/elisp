...
(defun brnm-find-element-in-nth-paragraph (num regexp)
  ...
  "Goes to the n'th paragraph separated by 2 blank lines, and the
   ...
   Previou documentation, for previous, more specific, function:
    ...
    3 is equivalent to 300.
   "
  (let ((paragraph-num 0) (paragraph-sub-num 0) (link-num 0))
    (if (= (point-max) 1)
	(insert " ")
      (goto-char (point-min))
	...
	(if (< num 10000)
	  ...
	  (setq paragraph-num (/ num 10000))
	  (setq paragraph-sub-num (/ (mod num 10000) 100))
	  (setq link-num (mod (mod num 10000) 100)))))
    

    (search-forward-regexp "\n\n+\n" nil t (- paragraph-num 1))

    (search-forward "\n\n" nil t paragraph-sub-num)
    
    (search-forward-regexp regexp nil t (+ link-num 1))
    ;; (let (group-end-pos)
